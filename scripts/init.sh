...
docker exec cockroachdb1 cockroach sql --certs-dir=$CERTS_DIR --host=localhost:26257 --execute "CREATE USER $COCKROACH_USER WITH PASSWORD '$COCKROACH_PASSWORD';"
docker exec cockroachdb1 cockroach cert create-client $COCKROACH_USER --certs-dir=$CERTS_DIR --ca-key=$CERTS_DIR/ca.key

echo "Creating database..."
docker exec cockroachdb1 cockroach sql --certs-dir=$CERTS_DIR --host=localhost:26257 --execute "CREATE DATABASE $COCKROACH_DATABASE;"
docker exec cockroachdb1 cockroach sql --certs-dir=$CERTS_DIR --host=localhost:26257 --execute "GRANT ALL ON DATABASE $COCKROACH_DATABASE TO $COCKROACH_USER;"
